...
- **Key Components:**
    - `pipeline.py`: The central workflow engine. Orchestrates the process across multiple languages in parallel.
    - `gpu_services.py`: Resource-bound services (LLM, TTS) that manage GPU access via priority queues (Internal RPC).
